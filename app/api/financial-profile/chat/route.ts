...
export async function POST(request: Request) {
  const { conversationHistory = [], sessionId } = await request.json();
  ...
  const result = await generateText({
    model: openai('gpt-4o'),
    system: `You are a professional financial advisor assistant specialized in capturing comprehensive financial profiles from users.
    ...
    - 6. (goals) Financial goals (short term, long term)
